...
def process_once():
    ...
    try:
        ...
        predictive_hit = (
            forward_intent
            and (
                pred_near >= FORWARD_NEAR_THRESHOLD
                or pred_risk >= FORWARD_RISK_THRESHOLD
            )
        )
        ...
        if raw_obstacle:
            close_hit_streak = min(
                close_hit_streak + (2 if predictive_hit or emergency_close else 1),
                8,
            )
        ...
        print(
            ...
            f"near={near_score:.1f} nearEma={near_ema:.1f} vClose={very_close_score:.1f} predNear={pred_near:.1f} predRisk={pred_risk:.1f} hit={close_hit_streak} "
        ...
        publish_metrics(
            ...
            near_ema_value=near_ema,
            very_close_score=very_close_score,
            pred_near=pred_near,
            pred_risk=pred_risk,
            hit=close_hit_streak,
            cmd=manual_cmd,
        )
        ...
        if obstacle:
            ...
            if stop_stuck_since is None:
                ...
                # Last resort escape when car remains stopped near obstacle too long.
                send("B")
                publish_metrics(
                    state,
                    True,
