...
// ======================================================
// WiFi Credentials (Apna WiFi SSID aur Password)
// ======================================================
const char* ssid     = "sumit";
const char* password = "12345678";

// ======================================================
// Azure Server API Endpoints
// ======================================================
const char* controlUrl = "http://20.244.113.234/fpv_car/get.php";
const char* uploadUrl  = "http://20.244.113.234/fpv_car/cam/upload.php";

const uint32_t CONTROL_INTERVAL_MS = 30;  // 30ms polling
const uint32_t UPLOAD_INTERVAL_MS  = 110; // ~9 FPS video upload
...
void initCamera() {
  ...
  if (esp_camera_init(&config) == ESP_OK) cameraReady = true;
}
...
void loop() {
  ...
  // Poll Server for Motor Command (Every 30ms)
  if (now - lastControlAt >= CONTROL_INTERVAL_MS) {
    ...
    int code = http.GET();
    if (code == 200) {
      char cmd = parseJsonCommand(http.getString());
      Serial.write(cmd); // Send to Arduino UNO
    }
    http.end();
  }

  // Upload Camera Frame (Every 110ms)
